...
public:
	...
	bool setvID(int ID0,int ID1,int ID2,const vector<Cvertex>&vlist){//前端返回是否构成三角形
		vID[0]=ID0;
		vID[1]=ID1;
		vID[2]=ID2;
		//计算法向量
		float p01[4],p02[4];
		sub(vlist[vID[1]].v,vlist[vID[0]].v,p01);
		sub(vlist[vID[2]].v,vlist[vID[0]].v,p02);
		cross(p01,p02,norm);
		if(normalize(norm)==0){//三点共线，不构成三角形
			return false;
		}
		return true;
	}
